# BTC_Framework/run_hybrid_strategy_BTC.py
# Walk-forward backtest for BTC-USD HYBRID STRATEGY
...
logger = setup_logger("walk_forward_engine_hybrid_BTC", log_file="walk_forward_engine_hybrid_BTC.log")
...
def walk_forward_backtest(
    ...
    # Save results with HYBRID suffix
    signals_df = pd.DataFrame(signals).set_index("Date")
    equity_df = pd.DataFrame(equity).set_index("Date")

    signals_out = RESULTS_DIR / f"signals_hybrid_BTC.csv"
    equity_out = RESULTS_DIR / f"equity_curve_hybrid_BTC.csv"
    log_out = RESULTS_DIR / f"backtest_log_hybrid_BTC.json"
    ...
    trades_out = RESULTS_DIR / f"trades_hybrid_BTC.csv"
...
if __name__ == "__main__":
    logger.info("===== WALK-FORWARD BACKTEST ENGINE (HYBRID STRATEGY - BTC) STARTED =====")
    ...
    logger.info("===== WALK-FORWARD BACKTEST ENGINE (HYBRID STRATEGY - BTC) FINISHED =====")
